...
import type { PersistWithdrawRequest } from './withdrawRecord';
...
export async function persistWithdraw(body: PersistWithdrawRequest): Promise<void> {
	const response = await fetch(resolve('/withdraw' as Pathname), {
		method: 'POST',
		headers: { 'content-type': 'application/json' },
		body: JSON.stringify(body)
	});
