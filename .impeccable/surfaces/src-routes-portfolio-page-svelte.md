...
Memorable moment: opened backed horse at call-scale; DEPOSIT or WITHDRAW chips pick the direction; then amount, one action (stamp or outlined redeem).

Constraints: overlay is `?horse=` on `/portfolio`, not a second route. Deposit stamp stays the raster; Withdraw is scratched-style outline. One share-redeem (capital + PnL together). Copy through Paraglide. Do not ship invented overlay slogans, red losses, or USDC from the decision comp.
...
- Do not literalize: “Guardian Protocol / Secure Yield”, red loss figures, dollar/USDC marks, a second filled stamp.
